...
def vcr_config() -> dict:
    ...
    config['before_record_request'] = scrub_body_request
    config['before_record_response'] = scrub_body_response
...
def scrub_body_response(response: dict) -> dict:
	if 'token' in response['body']['string'].decode(): 
		response['body']['string'] = json.dumps(dict(code=0,token=dict(access_token='123'))).encode('utf-8')
	return response
